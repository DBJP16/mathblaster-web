...
function create() {
    ...
    this.physics.add.collider(bullets, answerShips, bulletHitShip, null, this);

    this.spaceKey = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE);

    this.input.keyboard.on('keydown-SPACE', () => {
        if (!gameOver) {
            let bullet = bullets.create(player.x, player.y - 20);
            if (bullet) {
                this.physics.world.enable(bullet);
                bullet.setSize(8, 16);
                bullet.setFillStyle(0xffffff);
                bullet.body.setVelocityY(-400);
                console.log("Bullet fired from:", bullet.x, bullet.y);
            }
        }
    });
...
// Add this function for bullet-ship collision
function bulletHitShip(bullet, ship) {
    console.log("Collision detected between bullet and ship");
    bullet.destroy();
