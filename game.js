...
class AnswerShip extends Phaser.GameObjects.Triangle {
    constructor(scene, x, number) {
        super(scene, x, 50, 25, 0, 0, 50, 50, 0xFF0000);
    ...
    update() {
        this.x += this.speedX;
        this.y += this.speedY;
        console.log("Ship update: y=", this.y, "shootTimer=", this.shootTimer);
        ...
        if (this.shootTimer <= 0 && enemyBullets.getLength() < 20) {
            console.log("Enemy shooting attempt!");
...
function startGame(scene) {
    ...
    player = scene.add.triangle(400, 540, 25, 0, 0, 50, 50, 0xFFFF00);
