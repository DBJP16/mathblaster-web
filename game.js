...
let score = 0, lives = 3, gameOver = false, question = null, gameStarted = false; // Add gameStarted flag
...
function update() {
    if (!playerName || !gameStarted) return; // Wait for name and game start
...
function profileScreen(scene) {
    ...
    function handleKeyDown(event) {
        ...
        if (event.key === 'Enter' && playerName) {
            ...
            prompt.destroy();
            instr.destroy();
            scene.input.keyboard.off('keydown', handleKeyDown); // Clean up
            startGame(scene);
...
function startGame(scene) {
    ...
    bullets = scene.physics.add.group(); // Reassign for fresh start
    ...
    spawnQuestion(scene);
    gameStarted = true; // Set flag after initialization
}
