...
function create() {
    ...
    const scene = this; // Capture scene context
...
function update() {
    ...
    if (!gameOver) {
        ...
        answerShips.getChildren().forEach(ship => {
            ship.update(this); // Pass scene to update
            if (ship.y > 540) {
                lives--;
...
class AnswerShip extends Phaser.GameObjects.Triangle {
    constructor(scene, x, number) {
        super(scene, x, 50, 25, 0, 0, 50, 0xFF0000); // Top, left, right points
    ...
    update(scene) {
        ...
        if (this.shootTimer <= 0 && enemyBullets.getLength() < 20) {
            ...
            const bullet = scene.physics.add.rectangle(this.x, this.y + 50, 5, 10, 0xFF0000);
