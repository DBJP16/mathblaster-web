...
class AnswerShip extends Phaser.GameObjects.Triangle {
    constructor(scene, x, number) {
        super(scene, x, 50, // Top point (nose)
              x - 25, 75,  // Left base (angled wing)
              x + 25, 75,  // Right base (angled wing)
              0xFF0000);   // Red color
        scene.add.existing(this);
        scene.physics.add.existing(this);
        this.number = number;
        this.speedY = 1;
        this.text = scene.add.text(x + 15, 90, number.toString(), fontStyle);
        console.log("Ship created at x=", x, "number=", number);
    }

    update() {
        // No enemy bullets—pure movement
    }
