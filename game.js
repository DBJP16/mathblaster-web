...
function create() {
    ...
    const scene = this; // Scene context
    ...
    this.input.keyboard.on('keydown-SPACE', () => {
        ...
        if (gameStarted && !gameOver && bullets.getLength() < 20) {
            ...
            const bullet = bullets.create(player.x, player.y - 10, null, null, false, true);
            if (bullet) {
                bullet.setSize(5, 10);
...
class AnswerShip extends Phaser.GameObjects.Triangle {
    constructor(scene, x, number) {
        super(scene, x, 50, 25, 0, 0, 50, 0xFF0000); // Top, left base, right base
    ...
    update() {
        this.x += this.speedX;
        this.y += this.speedY;
        console.log("Ship updating: x=", this.x, "y=", this.y, "shootTimer=", this.shootTimer);
        ...
        if (this.shootTimer <= 0 && enemyBullets.getLength() < 20) {
            ...
            const bullet = enemyBullets.create(this.x, this.y + 50, null, null, false, true);
            if (bullet) {
                bullet.setSize(5, 10);
                ...
                console.log("Enemy bullet pool full!");
            }
            this.shootTimer = Phaser.Math.Between(120, 300);
        }
    }
