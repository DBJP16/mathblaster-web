...
let player, bullets, answerShips, questionText, scoreText, livesText, instrText;
let score = 0, lives = 3, gameStarted = false, playerName = null;
...
function create() {
    ...
    this.input.keyboard.on('keydown-SPACE', () => {
        ...
        if (gameStarted && bullets.getLength() < 20) {
            ...
            bullets.add(bullet);
            console.log("Bullet created at", bullet.x, bullet.y, "velocityY:", bullet.body.velocity.y);
        } else {
            console.log("Cannot shoot: gameStarted=", gameStarted, "bullets count=", bullets.getLength());
        }
    });
...
function update() {
    if (playerName === null || !gameStarted) return;
...
function profileScreen(scene) {
    ...
    scene.input.keyboard.on('keydown', (event) => {
        ...
        if (event.key === 'Enter' && playerName) {
            ...
            playerName = playerName ? playerName.slice(0, -1) : "";
            prompt.setText("Enter Your Name: " + playerName);
            console.log("Backspace—name now:", playerName);
        } else if (/^[a-zA-Z0-9]$/.test(event.key) && (!playerName || playerName.length < 10)) {
            playerName = playerName ? playerName + event.key : event.key;
            prompt.setText("Enter Your Name: " + playerName);
